...
export class AudioPlusEngine {
  ...
  setMetronome(on: boolean, bpm: number, beatsPerMeasure: number) {
    ...
    if (on) {
      if (this.clickTimer === null) {
        const ctx = this.getCtx();
        this.currentBeat = 0;
        this.nextClickTime = ctx.currentTime;
        this.scheduleClicks(bpm);
      }
    } else {
      if (this.clickTimer !== null) {
  ...
  private scheduleClicks(bpm: number) {
    ...
    while (this.nextClickTime < ctx.currentTime + SCHEDULE_AHEAD_S) {
      this.playClick(
        this.nextClickTime,
        this.currentBeat % this.beatsPerMeasure === 0,
      );
  ...
  async startRecording(
    ...
    const countInDuration = countIn ? beatsPerMeasure * (60 / bpm) : 0;
    const tracksStartAt = ctx.currentTime + countInDuration;

    // Count-in click plays if countIn is on; continues after if metronomeOn
    if (countIn || metronomeOn) {
      this.currentBeat = 0;
      this.nextClickTime = ctx.currentTime;
      this.scheduleClicks(bpm);
    }
  ...
  ): Promise<{ audioData: ArrayBuffer; startOffset: number }> {
